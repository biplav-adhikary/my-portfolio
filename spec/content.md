...
1. **HighRadius (2020–2023)**: Foundational growth from intern to full-stack contributor on a Fortune 500-facing receivables platform. Three promotions in 2.5 years (Intern → ASE I → ASE II). Core work included: end-to-end workflow automation for the autonomous collections process (React + Spring REST APIs + data layer), a custom email editor with AMP and Microsoft Actionable Message integration replacing manual communication workflows, Java-based ETL agents for scheduled data processing and storage, Elasticsearch integration reducing search latency to sub-100ms across large datasets, and a reusable design system adopted across multiple product surfaces. Mentored junior interns on technical solutioning and business alignment. Recognized with two peer awards for engineering quality and delivery. This chapter is about learning to build real products under real constraints — messy data, complex enterprise workflows, and users who don't tolerate broken tools.

2. **Sage (2023–Present)**: Ownership and architectural thinking at scale. Designed and maintained micro-frontend architecture using Nx, with a shared publishable library externalized for cross-repository reuse. Built the Intacct Payment Portal from scratch in collaboration with integration teams — API contract design through frontend — and shipped Online Payments to production, a revenue-generating feature enabling payment processor fees. Migrated the self-service application from Webpack to Rspack, significantly improving startup/load times and developer workflow. Standardized accessibility processes across the product (automated audits, navigation validation, screen reader checks, WCAG 2.1AA compliance). Built a customer-facing AI chatbot for feedback and support interactions. Introduced AI-driven tooling for release automation, documentation generation, translation, and JIRA story preparation — reducing repetitive overhead across the team. Led cross-functional design discussions to align product direction across squads. Supported cross-team enablement for ERP and payments integrations. Drove ongoing technical debt reduction and architectural refactoring. Improved documentation practices for delivered features to ensure long-term knowledge retention. Promoted to Engineer (Jan 2025) with expanded scope over end-to-end delivery and high-level architecture. Growth from frontend feature delivery to cross-stack platform ownership.
